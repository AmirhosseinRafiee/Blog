...
from .serializers import RegistrationSerializer, ChangePasswordSerializer, ProfileSerializer, CustomAuthTokenSerializer, CustomTokenObtainPairSerializer, ActivationResendSerializer
...
class ActivationApiView(APIView):
  ...
  def get(self, request, token, *args, **kwargs):
    ...
    if user.is_verified:
      return Response({'detail': 'your account has already been verified'}, status=status.HTTP_403_FORBIDDEN)
    user.is_verified = True
    user.save()
    return Response({'detail': 'your account have been verified successfully'}, status=status.HTTP_204_NO_CONTENT)
...
class ActivationResendApiView(GenericAPIView):
  ...
  def post(self, request, *args, **kwargs):
    ...
    email_obj = EmailMessage('email/activation.tpl', {'token': token}, 'admin@admin.com', to=[user.email])
    EmailTread(email_obj).start()
    return Response({"detail":"user activation resend successfully"}, status=status.HTTP_204_NO_CONTENT)
...
class TestSendEmail(APIView):
  ...
  def get(self, request, *args, **kwargs):
    ...
    token = self.get_access_token_for_user(user)
    email_obj = EmailMessage('email/hello.tpl', {'token': token}, 'admin@admin.com', to=[user.email])
